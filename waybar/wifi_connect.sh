...
#choose SSID
network=$(iwctl station wlan0 get-networks | tail -n +5 | sed 's/[>]//;s/\x1b\[[0-9;]*m//g;s/^[[:space:]]*//' | awk '{print $1}'| rofi -dmenu -theme ~/.config/rofi/wifi_list/config.rasi -p "Enter SSID")
...
#prompt for password
password=$(rofi -dmenu -theme ~/.config/rofi/wifi_password/config.rasi -p "Enter password for $network: ")
